...
if [ ! -f "$database" ]; then
    ...
    if [[ -z $option ]]; then
		...
		if cat /etc/passwd |grep -w $usuario > /dev/null; then
			...
			then
				...
				then
					...
					then
						...
						tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "Limit applied user $usuario to $sshnum " ; tput sgr0
						sleep 2
						exit
					fi
				fi
			...
			tput setaf 7 ; tput setab 1 ; tput bold ; echo "" ; echo "The  $usuario user was not found" ; echo "" ; tput sgr0
			exit 1
		fi
	fi
fi
